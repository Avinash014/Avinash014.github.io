...
function save(){
    ...
    for(let index = 0; index < selectedOption.length; index++){
        if(selectedOption[index].checked==true)selectedIndex = index+1;
    }
...
async function endTest(){
    ...
    for(var i = 0;i<totalQuestion; i++){
        ...
        if(qaObject.selected != -1 ){
            if(qaObject.selected == qaObject.answer)countCorrect++;
            else countIncorrect++;
        }
    }
    ...
    await setCookie("userMarks",JSON.stringify(markObj),100);
